...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!result.ok) return res.status(400).json({ error: result.description });
    ...
    fileStore.set(randomId, {
      telegramUrl, name: req.file.originalname, size: req.file.size,
      uploadedAt: new Date().toISOString(), mimeType: req.file.mimetype
    });
    
    res.json({ success: true, url: `https://${req.get('host')}/f/${randomId}`, id: randomId });
  } catch (error) {
  ...
  try {
    ...
    if (message.text === '/start') {
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: chatId, text: '📁 Kirim file apa aja (gambar, video, dokumen, zip, apk) ke bot ini, nanti saya kasih link download.' })
      });
      return;
    }
    
    let fileId = null, fileName = 'file', mimeType = 'application/octet-stream';
    ...
    if (fileId) {
      ...
      fileStore.set(randomId, {
        telegramUrl, name: fileName, size: fileSize,
      ...
      await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: chatId, text: `✅ Upload berhasil!\n\n🔗 Link: ${shortUrl}\n\n📄 Nama: ${fileName}\n📦 Ukuran: ${(fileSize/1024).toFixed(2)} KB` })
      });
    }
  } catch (error) {
...
// ========== WEB VIEW BAGUS (FONT AWESOME) ==========
app.get('/f/:id', async (req, res) => {
  const file = fileStore.get(req.params.id);
  if (!file) {
    return res.status(404).send(`<!DOCTYPE html><html><head><title>Not Found</title><style>body{background:#0f172a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:system-ui;}</style></head><body><div style="background:#1e293b;padding:2rem;border-radius:1rem;text-align:center;"><h1>❌ File Tidak Ditemukan</h1><a href="/" style="color:#60a5fa;">Kembali</a></div></body></html>`);
  }
  ...
  if (fileType === 'text') {
    ...
    previewHtml = content ? `<pre>${escapeHtml(content)}</pre>` : `<p><i class="fas fa-eye-slash"></i> Preview tidak tersedia</p>`;
  } else if (fileType === 'image') {
    const base64 = await getImageBase64(file.telegramUrl);
    previewHtml = base64 ? `<img src="${base64}" alt="preview">` : `<p><i class="fas fa-eye-slash"></i> Preview tidak tersedia</p>`;
    ...
    previewHtml = `<div style="color:#64748b; text-align:center;"><i class="fas fa-file" style="font-size:2rem; margin-bottom:10px; display:block;"></i> Preview tidak tersedia. Silakan download untuk melihat.</div>`;
  }
...
app.get('/api/files', (req, res) => {
  const files = Array.from(fileStore.entries()).map(([id, file]) => ({
    id, name: file.name, size: file.size, uploadedAt: file.uploadedAt
  }));
  res.json(files);
});
